import numpy as np
import matplotlib.pyplot as plt
import seaborn as sns

__all__ = ["plot_discrete_states_with_speed"]

def plot_discrete_states_with_speed(
    z_states,          # (T,) integer labels
    time_vec,          # (T,) seconds
    speed,             # (T,) raw or cleaned speed
    shock_times=None,  # None | 0/1 vector | timestamp list
    *,
    palette="Set1",
    lw=4,
    title=None,
    figsize=(12, 3),
    speed_color="black",
    speed_alpha=0.6,
    speed_scale=0.8,
    speed_offset=0.3
):
    """Plot discrete state runs with overlaid speed on the same axis."""

    z_states = np.asarray(z_states, dtype=int)
    time_vec = np.asarray(time_vec, dtype=float)
    speed = np.asarray(speed, dtype=float)
    
    assert z_states.shape == time_vec.shape == speed.shape, "All inputs must be same length"

    # Normalize speed for overlay
    speed_norm = (speed - speed.min()) / (speed.max() - speed.min())

    K = z_states.max() + 1
    colors = sns.color_palette(palette, n_colors=K)

    # Segment state runs
    change_idx = np.where(np.diff(z_states) != 0)[0]
    starts = np.concatenate(([0], change_idx - 1))
    ends   = np.concatenate((change_idx, [len(z_states) - 1]))
    starts = np.clip(starts, 0, len(z_states) - 1)
    ends   = np.clip(ends,   0, len(z_states) - 1)

    # Plot
    fig, ax = plt.subplots(figsize=figsize)

    # Discrete state horizontal bars
    for s, e in zip(starts, ends):
        k = z_states[s]
        ax.hlines(k, time_vec[s], time_vec[e],
                  color=colors[k], lw=lw, alpha=.8)

    # Overlay speed trace (offset above state lines)
    ax.plot(time_vec,
            speed_norm * speed_scale + K - 1 + speed_offset,
            color=speed_color, alpha=speed_alpha, label="Speed (normalized)")

    # Footshock markers
    if shock_times is not None:
        shock_times = np.squeeze(shock_times)
        if shock_times.shape == z_states.shape:
            s_times = time_vec[shock_times.astype(bool)]
        else:
            s_times = np.asarray(shock_times, dtype=float)
        for ts in s_times:
            if time_vec[0] <= ts <= time_vec[-1]:
                ax.axvline(ts, color="k", ls="--", lw=1)

    # Axis cosmetics
    ax.set_ylim(-0.5, K + 0.8)
    ax.set_yticks(range(K))
    ax.set_yticklabels([f"state {k}" for k in range(K)])
    ax.set_xlabel("time (s)")
    ax.set_title(title or "Discrete states with speed overlay")
    ax.legend()
    sns.despine(ax=ax, top=True, right=True)
    plt.tight_layout()
    plt.show()
