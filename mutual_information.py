"""mi_helper.py – Mutual Information utilities
Simplified single‑function helper to compute MI between rSLDS latents and
any 1‑D behavioural signal.
"""
...
def phase_shuffle(sig: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    """Surrogate with same power spectrum, random phase."""
    fft = np.fft.rfft(sig)
...
def _mi_surrogates(x: np.ndarray, y: np.ndarray, *, shuffle: str, n: int, seed: int) -> Tuple[float, float, float]:
    rng = np.random.default_rng(seed)
    k   = _adaptive_k(len(x))
    raw = _mi(x, y, k, seed)
    null = np.empty(n)
    for i in range(n):
        y_surr = circular_shift(y, rng.integers(len(y))) if shuffle == "circular" else phase_shuffle(y, rng)
...
def latent_signal_mi(
    *,
    ...
    shuffle: Literal["circular", "strict"] = "circular",
    n_shuffle: int = 500,
    random_state: int = 0,
) -> np.ndarray:
    """Return structured array of MI results for every latent dim."""
    ...
    for d in range(latents.shape[1]):
        ...
        raw, thr, p = _mi_surrogates(lat_win, sig_win, shuffle=shuffle, n=n_shuffle, seed=rng_root.integers(2**32-1))
